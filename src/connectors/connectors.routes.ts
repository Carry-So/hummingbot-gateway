...
import { GlobianceConfig } from './globiance/globiance.config';
    ...
    asyncHandler(async (_req, res) => {
      res.status(200).json({
        connectors: [
          ...
          {
            name: 'globiance',
            trading_type: GlobianceConfig.config.tradingTypes,
            available_networks: GlobianceConfig.config.availableNetworks,
          },
          {
            name: 'perp',
